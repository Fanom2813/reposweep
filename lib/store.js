...
export function clean(projectId) {
  ...
  // Remove project if nothing left to clean, otherwise update it
  if (reclaimableBytes === 0) {
    projects.value = p.filter(x => x.id !== projectId);
  } else {
...
export function getStats() {
  ...
  return {
    ...
    largestProjects: p
      ...
      .slice(0, 10)
      .map(x => ({ name: x.name, path: x.path, type: x.type, typeId: x.typeId, devicon: x.devicon, size: x.reclaimableBytes })),
  };
}
