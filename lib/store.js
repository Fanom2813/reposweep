...
export class Store {
  ...
  selectRoot(path) {
    if (this.selectedRoot === path) return;
    this.selectedRoot = path;
  ...
  async scan() {
    ...
    const result = await scanRoot(this.selectedRoot, this.settings, () => {
      this.notify();
    }, this._scanEpoch);
  ...
  clean(projectId) {
    ...
    // Rescan the cleaned project (pass type def + exclusions)
    const typeDef = Registry.getById(project.typeId);
    const rescannedTargets = findCleanupTargets(project.path, typeDef, this.settings.exclusions);
